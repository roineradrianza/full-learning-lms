<v-row class="px-4 px-md-16 pb-16 mt-12" course_id="<?php echo $course_id ?>" ref="course_container">
    ...
    <?php echo new Template('course/parts/enrollment_successful', $data) ?>
    <?php endif?>
...
;?> d-flex align-center" src="<?php echo $featured_image ?>" width="100%"
        ...
        <v-tabs class="course-tabs border-top" v-model="course_tab" fixed-tabs centered show-arrows ref="tabs_section">
            ...
            <v-tab class="pt-6 white--text font-weight-light" href="#digital-expo" v-if="sponsors.length">Expo Digital
            </v-tab>

            <v-tab-item class="px-14 pt-5 description-container ql-editor fl-background" value="description">
                <v-row class="d-flex justify-center">
                    <v-col class="p-0" cols="8" md="2">
                        <v-img src="<?php echo SITE_URL ?>/public/img/full-learning-logo.png"></v-img>
                    </v-col>
                </v-row>
                <v-img></v-img>
            ...
            <v-tab-item class="px-14 py-10" value="instructors">
                <?php echo new Template('course/tabs/instructors', $instructors) ?>
            </v-tab-item>
            <?php endif?>

            <v-tab-item class="px-14 py-10" value="curriculum">
                <?php echo new Template('course/tabs/curriculum', [
            ...
            <v-tab-item class="px-14 py-10" value="faq">
                <?php echo new Template('course/tabs/FAQ', json_decode($meta['faq'], true)) ?>
            </v-tab-item>
            <?php endif?>

            <?php if (isset($manage_course) && $manage_course): ?>
            <v-tab-item class="px-14 py-10" value="list">
                <?php echo new Template('course/tabs/list') ?>
            </v-tab-item>
            <?php endif?>

            <v-tab-item class="px-14 py-10" value="certified">
                <?php echo new Template('course/tabs/certified', $data) ?>
            </v-tab-item>

            <v-tab-item class="px-14 py-10" value="comments">
                <?php echo new Template('course/tabs/comments') ?>
            </v-tab-item>

            <v-tab-item class="px-3" value="digital-expo" v-if="sponsors.length > 0">
                <?php echo new Template('course/tabs/digital-expo', $data) ?>
            </v-tab-item>
        </v-tabs>
        <v-sheet class="gradient" height="2vw"></v-sheet>
    </v-col>
    <?php echo new Template('course/parts/sidebar', $data) ?>
</v-row>