<v-row class="pl-6 px-md-16 d-flex justify-center">
    ...
    <v-col cols="12" md="3" sm="12">
        <v-card :loading="loading" class="my-12 course-card"
            href="<?php echo SITE_URL . '/courses/' . $course['slug'] ?>" max-width="95%">

            <v-img width="100vw" height="27vh" class="align-end" src="<?php echo $course['featured_image'] ?>">
                <v-card-title class="category-label float-left mb-6 font-weight-bold">
                    <?php echo $course['category'] ?>
                </v-card-title>
            </v-img>

            <v-card-title class="text-h6 font-weight-normal no-word-break"><?php echo $course['title'] ?></v-card-title>

            <v-card-text class="grey--text text--lighten-1">
                <?php if (!empty($course['description']) ) : ?>
                <div class="truncate-overflow mb-0"><?php echo $course['description'] ?></div>
                <?php endif?>
            ...
            <v-card-text class="pb-md-10 pb-lg-5">
                ...
                <v-avatar class="teacher-course-avatar">
                    ...
                    <img src="<?php echo SITE_URL ?>/public/img/avatar-logo.png" width="20px" height="20px" alt="FL">
                    <?php else: ?>
                    <img src="<?php echo $course['avatar'] ?>" width="20px" height="20px"
                        alt="<?php echo $course['full_name'] ?>">
                    <?php endif?>
                </v-avatar>
                <?php if ($course['platform_owner']) : ?>
                ...
                <span class="font-weight-normal">Por </span><span class="subtitle-2 font-weight-bold">
                    <?php echo $course['full_name'] ?></span class="subtitle-2 text-primary">
                <?php endif?>
                <?php else: ?>
