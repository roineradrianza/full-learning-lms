				
				<v-row class="register-container">
					<v-col class="px-md-16 pt-16 white my-12 rounded-xl register-container mb-16" cols="8" offset="2" v-if="loading">
						<v-row>
							<v-col cols="6" v-for="i in 20">
								<v-skeleton-loader class="mt-3" type="text"></v-skeleton-loader>
							</v-col>
						</v-row>
					</v-col>
					<v-col class="px-md-16 pt-16 white my-12 rounded-xl register-container" cols="8" offset="2" v-else>
						<h3 class="text-h3">Registro</h3>
						<p class="text-h6 text-center mb-n1 pr-md-10">Usar: </p>
						<v-row class="d-flex justify-center mb-n2 pr-md-10">
							<v-btn class="ma-2 white--text padding- py-6" @click="googleSignIn">
					      <v-img class="myGoogleButton" src="<?php echo SITE_URL ?>/public/img/google-logo.png" width="1vw"></v-img>
					    </v-btn>
							<v-btn class="ma-2 white--text py-6" v-if="1 == 2">
					      <v-img src="<?php echo SITE_URL ?>/public/img/facebook-logo.svg" width="1vw"></v-img>
					    </v-btn>
						</v-row>
						<v-form ref="form" v-model="valid" lazy-validation>
							<v-row class="pr-md-10">

								<v-col cols="12" md="6">
									<label class="body-1 font-weight-thin pl-1">Nombre</label>
									<v-text-field type="text" name="first_name" v-model="form.first_name" class="mt-3 fl-text-input" :rules="validations.nameRules" filled rounded dense></v-text-field>
								</v-col>

								<v-col cols="12" md="6">
									<label class="body-1 font-weight-thin pl-1">Apellido</label>
									<v-text-field type="text" name="last_name" v-model="form.last_name" class="mt-3 fl-text-input" :rules="validations.nameRules" filled rounded dense></v-text-field>
								</v-col>

								<v-col cols="12" md="6">
									<label class="body-1 font-weight-thin pl-1">Usuario</label>
									<v-text-field type="text" name="username" v-model="form.username" class="mt-3 fl-text-input" :rules="validations.usernameRules" filled rounded dense></v-text-field>
								</v-col>

								<v-col cols="12" md="6">
									<label class="body-1 font-weight-thin pl-1">Email</label>
									<v-text-field type="email" name="email" v-model="form.email" class="mt-3 fl-text-input" :rules="validations.emailRules" filled rounded dense></v-text-field>
								</v-col>

								<v-col cols="12" md="6">
									<label class="body-1 font-weight-thin pl-1">Fecha de nacimiento</label>
		              <v-dialog ref="birthdate_dialog" v-model="birthdate_modal" :return-value.sync="form.birthdate" width="20vw">
                    <template v-slot:activator="{ on, attrs }">
                      <v-text-field class="mt-3 fl-text-input pt-select" v-model="form.birthdate" append-icon="mdi-calendar" readonly v-bind="attrs" v-on="on" :rules="validations.birthdateRules" filled rounded dense></v-text-field>
                    </template>
                    <v-date-picker v-model="form.birthdate" locale="es" scrollable>
                      <v-spacer></v-spacer>
                      <v-btn text color="primary" @click="birthdate_modal = false">
                        Cancelar
                      </v-btn>
                      <v-btn text color="primary" @click="$refs.birthdate_dialog.save(form.birthdate)">
                        OK
                      </v-btn>
                    </v-date-picker>
                  </v-dialog>
								</v-col>

								<v-col cols="12" md="6">
									<label class="body-1 font-weight-thin pl-1">Sexo</label>
                  <v-select class="mt-3 fl-text-input pt-select" v-model="form.gender" :items="gender" item-text="text" item-value="value" :rules="validations.genderRules" filled rounded dense></v-select>
								</v-col>

								<v-col cols="12" md="6">
									<label class="body-1 font-weight-thin pl-1">Télefono</label>
									<vue-tel-input-vuetify id="tel-input" class="mt-3 fl-text-input pt-select" v-model="form.meta.telephone" label='' mode="international" :inputoptions="{showDialCode: true}" :rules="validations.telephoneRules" placeholder="Ingresa un número de télefono" hint="Ej: +58 4245732965" persistent-hint @input="getInput" filled rounded dense></vue-tel-input-vuetify>
								</v-col>

								<template v-if="form.meta.student_type == 'medicine_student'">
									<v-col cols="12" md="6">
										<label class="body-1 font-weight-thin pl-1">Postgrado</label>
                  	<v-select class="mt-3 fl-text-input pt-select" v-model="form.meta.degree" :items="degrees" :rules="validations.selectRules" filled rounded dense></v-select>
									</v-col>

									<v-col cols="12" md="6">
										<label class="body-1 font-weight-thin pl-1">Nivel de Residente</label>
                  	<v-select class="mt-3 fl-text-input pt-select" v-model="form.meta.resident_level" :items="resident_levels" :rules="validations.selectRules" filled rounded dense></v-select>
									</v-col>
								</template>

								<template v-if="form.meta.student_type == 'medicine_teacher'">
									<v-col cols="12" md="6">
										<label class="body-1 font-weight-thin pl-1">Especialidad</label>
                  	<v-text-field type="text" name="teacher_degree" v-model="form.meta.teacher_degree" class="mt-3 fl-text-input" :rules="validations.requiredRules" filled rounded dense></v-text-field>
                  </v-col>

									<v-col cols="12" md="6">
										<label class="body-1 font-weight-thin pl-1">Años de graduado</label>
                  	<v-text-field type="text" name="ages_graduated" v-model="form.meta.ages_graduated" class="mt-3 fl-text-input" :rules="validations.agesGraduated" filled rounded dense></v-text-field>
									</v-col>

									<v-col cols="12" md="6">
										<label class="body-1 font-weight-thin pl-1">Centro de Trabajo</label>
                  	<v-text-field type="text" name="work_place" v-model="form.meta.work_place" class="mt-3 fl-text-input" :rules="validations.nameRules" filled rounded dense></v-text-field>
									</v-col>
								</template>

								<v-col cols="12" md="6">
									<label class="body-1 font-weight-thin pl-1">País</label>
                  <v-select class="mt-3 fl-text-input pt-select" v-model="form.country_selected" :items="countries" item-text="name" item-value="id" v-on:change="filterStates" :rules="validations.countryRules" filled rounded dense></v-select>
								</v-col>

								<v-col cols="12" md="6">
									<label class="body-1 font-weight-thin pl-1">Provincia o Ciudad</label>
                  <v-select class="mt-3 fl-text-input pt-select" v-model="form.state_selected" :items="country_states" item-text="name" item-value="id" v-on:change='getLocation' :rules="validations.countryStateRules" filled rounded dense></v-select>
								</v-col>

								<v-col cols="12" md="6">
									<label class="body-1 font-weight-thin pl-1">Contraseña</label>
									<v-text-field type="password" name="password" v-model="form.password" class="mt-3 fl-text-input" :rules="validations.passwordRules" filled rounded dense></v-text-field>
								</v-col>

								<v-col cols="12" md="6">
									<label class="body-1 font-weight-thin pl-1">Confirmar contraseña</label>
									<v-text-field type="password" name="password_confirm" v-model="form.password_confirm" class="mt-3 fl-text-input" filled rounded dense></v-text-field>
								</v-col>
								<?php echo new Template('components/alert') ?>
								<v-col cols="12">
									<v-row class="px-10 d-flex align-center">
										<v-col cols="12" md="4"></v-col>
										<v-col cols="12" md="4">
											<v-btn class="white--text secondary font-weight-bold rounded-pill mb-6 mt-4 py-6" @click="save" :disabled="!valid" :loading="save_loading" block>Registrarse</v-btn>
										</v-col>
									</v-row>									
								</v-col>

							</v-row>
						</v-form>
					</v-col>
				</v-row>