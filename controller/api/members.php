...
switch ($method) {
	...
	case 'sign-in':
		...
		if (isset($data['gid']) && !empty($data['gid'])) {
			...
			if (empty($existsGid)) {
				...
				if (!$result) $helper->response_message('Error', 'No se logró procesar el registro correctamente, intente de nuevo', 'error');
				if (isset($data['meta']) && !empty($data['meta'])) {
					foreach ($data['meta'] as $meta_key => $meta_value) {
						$meta = ['meta_name' => $meta_key, 'meta_val' => $meta_value, 'user_id' => $result]; 
						$user_meta->create($meta);
	...
	case 'update-avatar':
		if (empty($_POST) || empty($_SESSION['user_id'])) $helper->response_message('Advertencia', 'Ninguna información fue recibida', 'warning');
		$data = $_POST;
		...
		if (move_uploaded_file($tmp_file, $path)) {
			...
			if (!$result) $helper->response_message('Error', 'No se pudo subir la imágen de perfil, intente nuevamente', 'error');
		}
